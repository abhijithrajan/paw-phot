...
def do_plot(folder,ds9):
	...
	while (iteration < 2):
	  ...
	  N_orig_stars = N_ref_stars
	  bad_ref_list = []
	  best_refs = np.arange(0,N_ref_stars)
	  # Removing based on brightness

	  for i in range(N_ref_stars):
	    if (par.cut_offs[5] < ds9[i+1][4] or ds9[i][4] < par.cut_offs[4]):
	      satured.append(i)
 	    
	  non_saturated_refs = [x for x in np.arange(0,N_ref_stars) if x not in satured]
	  best_refs = non_saturated_refs
 	    
	  if len(non_saturated_refs) != N_ref_stars:
	    # Removing saturated refs
	    flux_r = [flux_r[i] for i in non_saturated_refs]
	    mag_r = [mag_r[i] for i in non_saturated_refs]
	    err_r = [err_r[i] for i in non_saturated_refs]
	    err_r_MAG = [err_r_MAG[i] for i in non_saturated_refs]
	    flux_r_norm = [flux_r_norm[i] for i in non_saturated_refs]
	    err_r_norm = [err_r_norm[i] for i in non_saturated_refs]
	    stddev = [stddev[i] for i in non_saturated_refs]
	    stddev_norm = [stddev_norm[i] for i in non_saturated_refs]
	    variance_norm = [variance_norm[i] for i in non_saturated_refs]
	    print 'Removing ',len(satured),' saturated refs'
	  N_ref_stars = len(non_saturated_refs)
	  ...
	  if (iteration == 0 and N_ref_stars > par.cut_offs[2]):
 	    ...
 	    if N_ref_stars > par.cut_offs[2]:
 	      starting_sample = par.cut_offs[2]
 	    else:
 	      starting_sample = par.cut_offs[2]-len(satured)
 	    
 	    for i in range(starting_sample):
 	      sim_refs.append(diff[i][1])
 	    dissimilar_refs = [x for x in np.arange(0,N_ref_stars) if x not in sim_refs]	    	
	    print 'Removing ',len(dissimilar_refs),' refs either to bright or too faint'	    
	  ...
	  N_ref_stars = len(best_refs)#N_ref_stars - len(bad_ref_list)      
	  ...
	  c.calc(folder,N_ref_stars,N_bad_refs,airmass,np_airmass,poly2d_AIRMASS,fwhm,JD,T_LC,err_target_w_mean_norm,norm_ref_star_flux_weight_sum_mean,REF_LCs,err_r_norm_sum,err_ref_Rs,master_ref,iteration,N_orig_stars,ds9)
	  iteration += 1
